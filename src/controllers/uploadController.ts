...
export const uploadRoomSnapshot = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Any participant can update snapshot when joining/leaving

    // Resize snapshot to 600px width while maintaining aspect ratio
    const metadata = await sharp(req.file.buffer).metadata();
    const aspectRatio = metadata.height! / metadata.width!;
    const resizedHeight = Math.round(600 * aspectRatio);

    const resizedBuffer = await sharp(req.file.buffer)
      .resize(600, resizedHeight, {
        fit: 'inside',
    ...
    const uploadPromise = new Promise<string>((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: 'room-snapshots',
          resource_type: 'image',
          format: 'jpg',
          public_id: `snapshot_${roomId}_${Date.now()}`,
        },
        (error, result) => {
          if (error) {
            reject(error);
    ...
    const snapshotUrl = await uploadPromise;

    // Update room with new snapshot URL
    ...
    console.error('Room snapshot upload error:', error);
    throw new AppError(500, 'UPLOAD_FAILED', 'Failed to upload room snapshot');
  }
};
